...
function BarChart({ data }) {
  if (!data || data.length === 0) {
    return <div className="chart-empty">No income data yet</div>;
  }
  ...
  return (
    <div>
      ...
      <div className="chart-legend">
        <span className="chart-legend-swatch income-swatch" />
        Income (Rs.)
      </div>
    </div>
  );
  ...
  return (
    <div className="income-card-panel">
      <div className="expense-card-title-row">
        <span className="expense-card-title">Income Trend (Last 6 Months)</span>
      </div>
      <BarChart data={data} />
    </div>
  );
